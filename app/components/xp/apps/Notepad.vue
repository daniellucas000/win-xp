...
const currentFile = computed(() =>
  fileId.value !== null ? fileSystem.getItem(fileId.value) : null
);
const fileName = computed(() => currentFile.value?.name || 'Sem título');

const displayTitle = computed(() => {
  const base = fileName.value.endsWith('.txt')
    ? fileName.value.replace('.txt', '')
    : fileName.value;
...
function updateCursorPosition() {
  const textarea = document.querySelector(
    `[data-window-id="${props.win?.id}"] textarea`
...
function saveAsFile() {
  showSaveDialog.value = true;
  saveFileName.value = fileName.value.endsWith('.txt')
    ? fileName.value
    : 'Documento.txt';
}
...
function confirmSaveAs() {
  ...
  const name = saveFileName.value.trim().endsWith('.txt')
    ? saveFileName.value.trim()
    : saveFileName.value.trim() + '.txt';
  const parentId =
    fileId.value !== null
      ? (fileSystem.getItem(fileId.value)?.parentId ?? 0)
      : 0;
  ...
  navigator.clipboard
    ...
    .then((clip) => {
      ...
      if (textarea) {
        ...
        text.value =
          text.value.substring(0, start) + clip + text.value.substring(end);
        nextTick(() => {
  ...
  <div class="notepad">
    <div class="notepad__menubar" role="menubar" aria-label="Menu">
      <div class="notepad__menu-wrapper">
        <button
          class="notepad__menu-item"
          :class="{ 'notepad__menu-item--active': activeMenu === 'file' }"
          role="menuitem"
        ...
        <div
          ...
          class="notepad__menu-dropdown"
          role="menu"
          @click.stop
        >
          <button
            class="notepad__menu-dropdown-item"
            role="menuitem"
            @click="newFile"
          >
            New
      ...
      <div class="notepad__menu-wrapper">
        <button
          class="notepad__menu-item"
          :class="{ 'notepad__menu-item--active': activeMenu === 'edit' }"
          role="menuitem"
      ...
      <div class="notepad__menu-wrapper">
        <button
          class="notepad__menu-item"
          :class="{ 'notepad__menu-item--active': activeMenu === 'format' }"
          role="menuitem"
      ...
      <div class="notepad__menu-wrapper">
        <button
          class="notepad__menu-item"
          :class="{ 'notepad__menu-item--active': activeMenu === 'view' }"
          role="menuitem"
    ...
    <label for="notepad-textarea" class="visually-hidden"
      >Conteúdo do documento</label
    >
    <textarea
      id="notepad-textarea"
      v-model="text"
      ...
      >
        <div class="notepad__save-dialog" @click.stop>
          ...
          <div class="notepad__save-actions">
            <button class="notepad__save-btn" @click="confirmSaveAs">
              Save
            </button>
            <button class="notepad__save-btn" @click="showSaveDialog = false">
              Cancel
            </button>
          </div>
        </div>
