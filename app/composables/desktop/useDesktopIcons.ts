...
const TRASH_FULL_ICON = '/images/xp/icons/recycle-bin-full.webp';
const TRASH_EMPTY_ICON = '/images/xp/icons/recycle-bin-empty.webp';
...
export function useDesktopIcons(options: UseDesktopIconsOptions) {
  ...
  function parseStoredIcons(
    key: string,
    extra?: Partial<DesktopIcon>
  ): DesktopIcon[] {
    try {
  ...
  function loadFromStorage() {
    ...
    const trashIcons = parseStoredIcons(STORAGE_KEYS.TRASH, {
      isDeleted: true,
    });
  ...
  const saveToStorage = useDebounceFn(() => {
    ...
    const userIcons = desktopIcons.value.filter(
      (icon) => !icon.isSystem && !icon.isDeleted
    );
